# -*- coding: utf-8 -*-
"""1_Preprocessing_0912.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1SFQ4ybL3gXaib20QEMMG_jCVjGqMYMV-
"""
...
### (1) alsfrs total slope
"""

# Filter first 3 month data
alsfrs_total = alsfrs_q_raw[['SubjectID','ALSFRS_Total', 'feature_delta']].sort_values(by=['SubjectID', 'feature_delta'], axis=0)
alsfrs_total_3mo = alsfrs_total.query('(feature_delta < 92) and (feature_delta >= 0)')

AT1 = alsfrs_total_3mo.groupby('SubjectID').agg(['first', 'last'])
AT1.reset_index(inplace=True)

AT1['interval'] = AT1[('feature_delta', 'last')] - AT1[('feature_delta', 'first')]
AT1 = AT1[AT1['interval']!=0]
AT1['difference'] = AT1[('ALSFRS_Total', 'last')] - AT1[('ALSFRS_Total', 'first')]

AT1['alsfrs_total_slope'] = AT1['difference']/ AT1['interval']
AT1 = AT1[['SubjectID', 'interval', 'alsfrs_total_slope']]
AT1.columns = ['SubjectID','interval','alsfrs_total_slope']

# data with time interval less than 30 days is regarded as missing data
slope_alsfrs_total_short = AT1[AT1['interval'] <30]
slope_alsfrs_total_long = AT1[AT1['interval']>=30]

slope_alsfrs_total_short['alsfrs_total_slope']=np.nan
alsfrs_total_slope = pd.concat([slope_alsfrs_total_short, slope_alsfrs_total_long], axis=0)
alsfrs_total_slope.drop(columns='interval', inplace=True)

# Convert scale 'days' to 'month'
alsfrs_total_slope['alsfrs_total_slope'] = alsfrs_total_slope['alsfrs_total_slope']*(365/12)
alsfrs_total_slope #6167 data

"""### (2) FVC"""

# Filter first 3 month data
fvc_3mo = fvc.query('(feature_delta < 92) and (feature_delta >= 0)')
fvc_3mo.columns = ['SubjectID','fvc','feature_delta']

# Extracting mean_fvc
mean_fvc = fvc_3mo.groupby('SubjectID').agg('mean')
mean_fvc = mean_fvc.reset_index()
mean_fvc.drop(columns='feature_delta', inplace=True)
mean_fvc #7217 data

# Calculate slope of fvc
F1 = fvc_3mo.groupby('SubjectID').agg(['first', 'last'])
F1.reset_index(inplace=True)

F1['interval'] = F1[('feature_delta', 'last')] - F1[('feature_delta', 'first')]
F1 = F1[F1['interval']!=0]
F1['difference'] = F1[('fvc', 'last')] - F1[('fvc', 'first')]

F1['fvc_slope'] = F1['difference']/ F1['interval']
F1 = F1[['SubjectID', 'interval', 'fvc_slope']]
F1.columns = ['SubjectID','interval','fvc_slope']

# data with time interval less than 30 days is regarded as missing data
slope_fvc_short = F1[F1['interval'] <30]
slope_fvc_long = F1[F1['interval']>=30]
slope_fvc_short['fvc_slope']=np.nan

fvc_slope = pd.concat([slope_fvc_short, slope_fvc_long], axis=0)

fvc_slope.drop(columns='interval', inplace=True)
fvc_slope # 5256 data

# Convert scale 'days' to 'month'
fvc_slope['fvc_slope'] = fvc_slope['fvc_slope']*(365/12)

# Merge mean_fvc & fvc_slope
fvc_summary = mean_fvc.merge(fvc_slope, on = 'SubjectID', how='outer')
fvc_summary.columns = ['SubjectID', 'fvc_mean', 'fvc_slope']
fvc_summary #5945 data
...
Creatinine_summary = mean_creatinine.merge(slope_creatinine, on = 'SubjectID', how='outer')
Creatinine_summary.columns = ['SubjectID', 'Creatinine_mean', 'Creatinine_slope']
Creatinine_summary #7712 data
...
feature_list = [demographics, als_hx, fvc_summary, Creatinine_summary, alsfrs_total_slope, mean_alsfrs_q, slope_alsfrs_q, weight_slope]
...
report_nulls(features) # {onset_delta / diag_delta / diag_minus_onset}-> 54.77%  {onset_site}-> 54.78%  {mean_alsfrs_q} -> 33.93%  {slope_alsfrs_q} -> 42.55%  of Total 9848 data
...
X_imputed.to_csv('/content/drive/MyDrive/Colab Notebooks/0911_X_imputed.csv')
...
len(fvc_filtered_extent) #6742 data
...
print("There are",len(Optimal_event_0['SubjectID'].unique()), "subjects whose status_opt = 0")
 # There are 1557 subjects whose status_opt = 1
 # There are 3105 subjects whose status_opt = 0
...
Optimal_Gas #4662 data

Optimal_Gas.to_csv('/content/drive/MyDrive/Colab Notebooks/0911_optimal_target.csv')
...
Real_Gas.to_csv('/content/drive/MyDrive/Colab Notebooks/0911_real_target.csv')
